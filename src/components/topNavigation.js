...
import { ToastContainer, toast, MDBNavbar, MDBNavbarBrand, MDBNavbarNav, MDBNavbarToggler, MDBCollapse, MDBNavItem, MDBNavLink, MDBIcon } from 'mdbreact';
...
class TopNavigation extends Component {
    ...
    addUser = (user) => {
        ...
        const userRef = db.collection('users').where('uid', '==', user.uid).get().then(
            snapshot => {
                let ok = 1;
                ...
                <MDBCollapse isOpen = { this.state.collapse } navbar>
                    ...
                    <MDBNavbarNav right>
                        ...
                        <MDBNavItem onClick={this.signInGoogle}>
                            <span className="nav-link navbar-link" ><MDBIcon fab icon="google" /></span>
                        </MDBNavItem> )}
                        {(!this.state.user &&
                        <MDBNavItem onClick={this.signInGithub}>
                            <span className="nav-link navbar-link" ><MDBIcon fab icon="github" /></span>
                        </MDBNavItem>)}
                        {(!this.state.user &&
                        <MDBNavItem>
                            <span className="nav-link navbar-link" rel="noopener noreferrer"><MDBIcon fab icon="twitter" /></span>
                        </MDBNavItem> )}
                        {(this.state.user && <MDBNavItem>
                            <span className="nav-link navbar-link" >Hello, {this.state.user.displayName}</span>
                        </MDBNavItem>)}
                        {( this.state.user && <MDBNavItem>
                            <span className="border border-light rounded mr-1 nav-link Ripple-parent" onClick={this.signOut}>logout</span>
                        </MDBNavItem>)}
                    </MDBNavbarNav>
                </MDBCollapse>
            </MDBNavbar>
